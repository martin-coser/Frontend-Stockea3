...
const Producto: React.FC = () => {
    ...
    const handleEliminarProducto= async (id: number) => {
        ...
        try {
            await axios.delete(`http://localhost:4000/producto/softDelete/${id}`);
                    ...
                    <tbody>
                        {productos.map((producto: any) => (
                            <tr key={producto.id} className="hover:bg-gray-200">
                                ...
                                {/* Iconos de modificar y eliminar */}
            ...
            {mostrarFormulario && (
                ...
                >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <select value={marca} onChange={(e) => setMarca(e.target.value)} className="w-full p-1.5 border border-gray-300 rounded">
                            <option value="">Seleccionar marca</option>
                            {marcasDisponibles.map((m) => (
                                <option key={m.id} value={m.id}>{m.nombre}</option>
                            ))}
                        </select>
                        <select value={categoria} onChange={(e) => setCategoria(e.target.value)} className="w-full p-1.5 border border-gray-300 rounded">
                            <option value="">Seleccionar categoría</option>
                            {categoriasDisponibles.map((c) => (
                                <option key={c.id} value={c.id}>{c.nombre}</option>
                            ))}
                        </select>
                        <select value={proveedor} onChange={(e) => setProveedor(e.target.value)} className="w-full p-1.5 border border-gray-300 rounded">
